...
void SynergyComponentInitializer::Init_SYS_ElementAmount(float strategyRatio_SYS) {
...
std::vector<Armor *> SynergyComponentInitializer::GetRandomArmorPointers(int amount) {
    ...
    std::sample(armorPointers.begin(), armorPointers.end(), std::back_inserter(randomArmorPointers), amount, gen);
...
std::vector<Weapon *> SynergyComponentInitializer::GetRandomWeaponPointers(int amount) {
    ...
    // Filter weapons that target ENEMY only
    ...
    // Adjust amount if not enough valid weapons
    int actualAmount = std::min(amount, (int)weaponPointers.size());

    std::vector<Weapon *> randomWeaponPointers;
    randomWeaponPointers.reserve(actualAmount);

    std::random_device rd;
    std::mt19937 gen(rd());

    std::sample(weaponPointers.begin(), weaponPointers.end(), std::back_inserter(randomWeaponPointers), actualAmount,
                gen);

    return randomWeaponPointers;
}

void SynergyComponentInitializer::Generate_UnitSynergyList(std::vector<Armor *> armorPtrs,
                                                           std::vector<Weapon *> weaponPtrs) {
...
void SynergyComponentInitializer::Init(float strategyRatio_SYS) {

    Reset();

    Init_SYS_ElementAmount(strategyRatio_SYS);
...
void SynergyComponentInitializer::Reset() {
    SynergyRule::unitSynergyMap.clear();

    for (auto &armor : *armorList) {
        armor.GetUnitSynergyIDs().clear();
    }
    for (auto &weapon : *weaponList) {
        weapon.GetUnitSynergyIDs().clear();
    }
}
