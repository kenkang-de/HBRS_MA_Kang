...
def get_rms_deviation_from_baseline(base_dir, game_components, bs_compare, cs_compare):
    sys = 0.0
    ...
    # Calculate RMS deviation between the gaps
...
def generate_strategy_color(bs, cs):
    # Map specific strategies to normalized lightness levels
    lightness_map = {
        (0.9, 0.1): 0.2,
        (0.8, 0.2): 0.4,
        (0.7, 0.3): 0.6,
...
def plot_rms_deviation_bar_chart(base_dir, strategies, save_path=None):
    ...
    rms_data = {f"BS={bs:.1f}, CS={cs:.1f}": [] for bs, cs in strategies}
    
    for bs, cs in strategies:
        strategy_label = f"BS={bs:.1f}, CS={cs:.1f}"
        print(f"Processing {strategy_label}...")
        
        for game_components in game_components_range:
            rms = get_rms_deviation_from_baseline(base_dir, game_components, bs, cs)
    ...
    for idx, (bs, cs) in enumerate(strategies):
        strategy_label = f"BS={bs:.1f}, CS={cs:.1f}"
        offset = (idx - n_strategies/2 + 0.5) * bar_width
        
        color = generate_strategy_color(bs, cs)
        
        ax.bar(positions + offset, rms_data[strategy_label], bar_width, 
               label=strategy_label, color=color)    
    avg_rms_per_component = []
    for i in range(n_components):
        values_at_component = [rms_data[f"BS={bs:.1f}, CS={cs:.1f}"][i] for bs, cs in strategies]
        avg_rms_per_component.append(np.mean(values_at_component))
    
    ax.plot(positions, avg_rms_per_component, color="#2CCC2C", linewidth=2.5, 
...
def main():
    ...
    for bs, cs in strategies:
        print(f"  BS={bs:.1f}, CS={cs:.1f}")
    
    # Generate the bar chart
    save_path = os.path.join(base_dir, "RMS_Deviation_BarChart.svg")
