...
def get_average_fitness_range(base_dir, game_components, bs, cs):
...
def plot_bs_cs_range_comparison(base_dir, bs_compare, cs_compare, save_path=None):
    ...
    for game_components in game_components_range:
        avg_lowest, avg_highest = get_average_fitness_range(base_dir, game_components, bs_baseline, cs_baseline)
    ...
    if x_baseline:
        ...
        plt.plot(x_baseline, y_baseline_high, linewidth=2, 
                color='black', linestyle='-', label=f"BS={bs_baseline:.1f}, CS={cs_baseline:.1f}")
    ...
    # Comparison: Custom BS/CS (blue)
    x_compare = []
    y_compare_low = []
    y_compare_high = []
    
    for game_components in game_components_range:
        avg_lowest, avg_highest = get_average_fitness_range(base_dir, game_components, bs_compare, cs_compare)
        if avg_lowest is not None and avg_highest is not None:
    ...
    if x_compare:
        ...
        plt.plot(x_compare, y_compare_high, linewidth=2, 
                color='#1f77b4', linestyle='-', label=f"BS={bs_compare:.1f}, CS={cs_compare:.1f}")
    ...
    plt.title(f'BS={bs_compare:.1f}, CS={cs_compare:.1f}\n', fontsize=14)
...
def main():
    ...
    print("Generating SVG files for BS/CS range comparisons (SYS=0.0):")
    ...
    for bs_compare, cs_compare in comparisons:
        # Create filename
        filename = f"Range_BS{bs_compare:.1f}_CS{cs_compare:.1f}_vs_Baseline.svg"
        save_path = os.path.join(base_dir, filename)
        
        print(f"Processing: BS={bs_compare:.1f}, CS={cs_compare:.1f}")
        
        # Generate and save the plot
        plot_bs_cs_range_comparison(base_dir, bs_compare, cs_compare, save_path=save_path)
